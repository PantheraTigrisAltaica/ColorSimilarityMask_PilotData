...
#refresh rates
rate_vars<- c( "participant", "realFrameRate")
...
rate = rate_data %>% group_by(ID) %>% summarise(Rate = mean(realFrameRate),) %>% ungroup()
...
#random test code
library(psych)
correlation_list = correlation_list %>% mutate(z = fisherz(abs(correlation_list$correlation)))
strong = correlation_list %>% filter(mask == 'strong')
weak = correlation_list %>% filter(mask == 'weak')
describe(strong$z)
describe(weak$z)

ggplot(temp_plot, aes(x = exp, y = meanZ, fill = mask, color = mask)) +
  geom_bar(stat = 'identity', position = 'dodge') + 
  geom_errorbar(mapping = aes(ymin = meanZ - 1.96 * SE, ymax = meanZ + 1.96 * SE),width = 0.2, color = 'black', position = position_dodge(0.9)) +
  labs(title = 'Average correlation across participants - fisher Z', x = 'exp', y = 'fisherZ')
...
ggplot(data = temp_plot) +
  geom_line(data = temp_plot %>% filter(Mask == 'strong'), aes(x = Colour, y = Dissimilarity, color = Exp, group = Exp)) +
  geom_line(data = temp_plot %>% filter(Mask == 'weak'), aes(x = Colour, y = Dissimilarity, color = Exp, group = Exp), linetype = 2) +
  scale_color_manual(name = "Exp",
                     values = c("Pilot 1" = 'black', "Pilot 2" = 'red', 'Pilot 3' = 'blue', 'Pilot 4' = 'green'), 
                     labels = c('Pilot 1', 'Pilot 2', 'Pilot 3', 'Pilot 4')) +
  labs(title = 'Diagonal dissimilarity for all data', x = 'Colour', y = 'Dissimilarity')
...
ggplot(temp_plot, aes(x = Exp, y = Dissimilarity, fill = Mask, color = Mask)) +
  geom_bar(stat = 'identity', position = 'dodge')+
  geom_errorbar(mapping = aes(ymin = Dissimilarity - 1.96 * SE, ymax = Dissimilarity + 1.96 * SE), width = 0.2, color = 'black', position = position_dodge(0.9)) +
  labs(title = 'Diagonal dissimilarity across participants', x = 'Exp', y = 'Dissimilarity')
